...
*/
function temperatureInF(num, tempType) {
  if(tempType === 'F'){
    return num + 'F';
  } else if(tempType === 'C'){
    let cTemp = temperatureCtoF(num);
    return cTemp + 'F';
  }
}
 ...
 * }
*/
function makePersonObject(id_num, f_name, f_email) {
  const Person = {
    id: id_num,
    name: f_name,
    email: f_email
...
function getLastCarInfo(inv) {
  ...
  const lastModel = inv[inv.length - 1].car_model;
